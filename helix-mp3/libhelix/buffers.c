...
#else
//#include "common.h"  all delay us ms
#endif
...
MP3DecInfo *AllocateBuffers(void)
{
  ...
  ClearBuffer((void*)&mp3DecInfo, sizeof(MP3DecInfo));
   ...
   // return mp3DecInfo_pointer;
	...
	mp3DecInfo_pointer = (MP3DecInfo*)p;p += sizeof(MP3DecInfo);
	
	ClearBuffer(mp3DecInfo_pointer, sizeof(MP3DecInfo));
		
	fh =  (FrameHeader *)     p;p += sizeof(FrameHeader);
	si =  (SideInfo *)        p;p += sizeof(SideInfo);
	sfi = (ScaleFactorInfo *) p;p += sizeof(ScaleFactorInfo);
	hi =  (HuffmanInfo *)     p;p += sizeof(HuffmanInfo);
	di =  (DequantInfo *)     p;p += sizeof(DequantInfo);
	mi =  (IMDCTInfo *)       p;p += sizeof(IMDCTInfo);
	sbi = (SubbandInfo *)     p;
