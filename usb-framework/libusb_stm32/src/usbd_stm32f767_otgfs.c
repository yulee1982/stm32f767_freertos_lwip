...
static void enable(bool enable)
{
  if(enable)
  {
    ...
    _WBS(OTG->GRSTCTL, USB_OTG_GRSTCTL_AHBIDL); //while((OTG->GRSTCTL & USB_OTG_GRSTCTL_AHBIDL) == 0)
...
#else
    OTG->GOTGCTL |= USB_OTG_GOTGCTL_BVALOEN | USB_OTG_GOTGCTL_BVALOVAL; //BVALID信号是AXI协议中用于指示写响应有效的信号
    OTG->GCCFG = USB_OTG_GCCFG_PWRDWN;
#endif
    /* Restart the Phy Clock */
    *OTGPCTL = 0; //USBx_PCGCCTL = 0U;

    /* soft disconnect device */
    OTGD->DCTL |= USB_OTG_DCTL_SDIS; //_BST(OTGD->DCTL, USB_OTG_DCTL_SDIS);
